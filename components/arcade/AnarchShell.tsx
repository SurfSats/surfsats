...
async function anarchFilesPresent() {
  try {
    const [html, js, wasm] = await Promise.all([
      fetch(ANARCH_BUILD_HTML, { method: "HEAD", cache: "no-store" }),
      fetch(ANARCH_BUILD_JS, { method: "HEAD", cache: "no-store" }),
      fetch(ANARCH_BUILD_WASM, { method: "HEAD", cache: "no-store" }),
    ]);
    return html.ok && js.ok && wasm.ok;
  } catch {
    return false;
  }
}
...
export function AnarchShell() {
  ...
  const boot = useCallback(async ({ creditCount = credits }: { creditCount?: number } = {}) => {
    ...
    try {
      const response = await fetch("/api/arcade/play", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ playerId, game: ANARCH_GAME_ID }),
      });
      const data = (await response.json()) as {
        error?: string;
        credits?: number;
  ...
  return (
    <div className={`anarch-shell${playing ? " is-playing" : ""}`}>
      {playing ? (
        <div className="anarch-run">
          <div className="anarch-run-bar">
            <p>ANARCH · drummyfish · CC0</p>
            <Link className="anarch-exit" href="/arcade#floor">
              EXIT
            </Link>
          </div>
          {buildOk ? (
            <div className="anarch-playfield">
              <iframe
                title="Anarch"
                src={ANARCH_BUILD_HTML}
                allow="autoplay; fullscreen; pointer-lock"
              />
            </div>
          ) : (
            <div className="anarch-missing">
              <p>Drop files here to boot.</p>
              <p>
                public/arcade/anarch/ needs anarch.html, anarch.js, and
                anarch.wasm from the official web build.
              </p>
            </div>
          )}
        </div>
        ...
        <>
          <div className="anarch-stage">
            ...
            {buildOk === false ? (
              <p className="anarch-copy">
                Wasm missing. Drop anarch.html, anarch.js, anarch.wasm into
                public/arcade/anarch/.
              </p>
            ) : null}
          </div>
